...
---
### [#15] 로컬 미리보기 & Git 원격 동기화
- 작성자(팀원): 마경림 (mkr)
- 목표: 디자인 변경을 로컬에서 확인하고 원격 저장소에 반영
- 에이전트에게 시킨 것(실제 프롬프트 핵심 인용):
  > "이제 local로 열어봐" / "이제 깃푸쉬해줘"
- 사용한 기법(있으면): (b 도구연동 — Vite dev 서버, git rebase)
- 결과: `npm run dev`로 로컬(http://localhost:5173) 미리보기 확인. 변경 일괄 커밋 후 push 시 원격이 8커밋 앞서 있어(behind 8) `git pull --rebase`로 통합, 푸시 도중 팀원 커밋이 추가돼 재rebase 후 충돌 없이 푸시 완료(main 동기화).
- 막힘 → 해결(있었다면): 첫 push가 non-fast-forward로 거부 → rebase 재실행으로 해결.

---
### [#16] 제출물 점검 & 영문 로그 사본 동기화
- 작성자(팀원): 마경림 (mkr)
- 목표: CHECKLIST 기준 제출물 누락 여부 점검
- 에이전트에게 시킨 것(실제 프롬프트 핵심 인용):
  > "submit/CHECKLIST.md로 빠진 게 없는지 확인해줘"
- 사용한 기법(있으면): (c 재사용산출물 — CHECKLIST 기반 자동 점검)
- 결과: 필수 6종(산출물/PROCESS_LOG/Before-After/assets/evidence/발표자료) 모두 충족 확인. 영문 제출 사본 `cool-cjm_mkr_PROCESS_LOG.md`가 5개 항목으로 오래돼 원본(14개)과 어긋나 있던 것을 발견해 최신 원본으로 동기화.
- 막힘 → 해결(있었다면): 영문 사본 미갱신 → 원본 복사로 일치시킴.

---
### [#17] (시도) LLM 기반 RFP 자동추출 — 성능 한계로 미채택
- 작성자(팀원): 마경림 (mkr)
- 목표: 제안요청서(PDF)에서 핵심 항목을 LLM으로 자동추출
- 에이전트에게 시킨 것(실제 프롬프트 핵심 인용):
  > "(agent2 추출 파이프라인) PDF 파싱 후 LLM으로 항목 추출"
- 사용한 기법(있으면): (b 도구연동 — PDF 파서 + LLM)
- 결과: **PDF 파싱 자체는 정상 동작**했으나, 이어지는 **LLM 추출/분석 단계의 성능(정확도·일관성)이 너무 낮아 실사용이 불가**해 해당 자동추출 방식은 채택하지 않음. (시도 기록으로 남김)
- 막힘 → 해결(있었다면): LLM 출력 품질 부족 → 본 방식 보류, 의사결정 대시보드 중심 산출물로 방향 전환.
