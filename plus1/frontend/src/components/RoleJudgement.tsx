...
export function RoleJudgement({ roleKey }: { roleKey: RoleConfig['key'] }) {
  ...
  // 행정부: 3분할 레이아웃 (슬라이더 | 레이더·판단 | 참여기관 사이드바)
  if (roleKey === 'admin') {
    return (
                ...
                <div className="overflow-x-auto">
                  <Table>
                    <THead>
                      <Tr>
                        <Th>기관</Th>
                        <Th className="text-center">신용</Th>
                        <Th className="text-center">역할</Th>
                        <Th className="text-center">계약</Th>
                      </Tr>
                    </THead>
                    <TBody>
                      {institutions.map((inst) => (
                        <Tr key={inst.name}>
                          <Td className="font-semibold text-blue whitespace-nowrap">{inst.name}</Td>
